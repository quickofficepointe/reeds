                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-semibold text-gray-900" id="totalQrCount">{{ $employees->total() }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                          ...
                          <dd class="text-lg font-semibold text-gray-900">{{ isset($departments) ? $departments->count() : 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div class="px-4 py-5 sm:p-6">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div class="col-span-1 md:col-span-6">
                    ...
                    <div class="relative rounded-md shadow-sm">
                        ...
                        <input type="text"
                               ...
                               placeholder="Search by name, code, department, unit...">
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <select id="departmentFilter" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <select id="unitFilter" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
            ...
            <div class="mt-4 flex justify-between items-center">
                <div>
                    <span id="searchResultsInfo" class="text-sm text-gray-500 hidden">
                        Found <span id="searchResultsCount">0</span> results
                    </span>
                </div>
                <div class="space-x-2">
                    <button onclick="resetFilters()" class="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        <i class="fas fa-redo mr-2"></i>
                        Reset Filters
                    </button>
                    <button onclick="applyFilters()" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
    ...
    <div class="bg-white shadow rounded-lg">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" id="qrCodesGrid">
                ...
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 employee-card"
                     data-id="{{ $employee->id }}"
                     data-department="{{ $employee->department_id }}"
                     data-unit="{{ $employee->unit_id }}"
                    ...
                    <div class="p-4">
                        ...
                        @if($employee->unit)
                        <p class="text-xs text-gray-500 mb-2">{{ $employee->unit->name }}</p>
                        @endif
            ...
            <div class="mt-6 border-t border-gray-200 pt-4" id="paginationContainer">
 ...
 async function generateEmployeeQRCode(employeeId) {
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/admin/employees/${employeeId}/qr-data`, {
            ...
            mode: 'same-origin' // 👈 Ensures request stays on same origin
        });

        if (!response.ok) {
            if (response.status === 401 || response.status === 419) {
                // Session expired - reload page to force re-login
        ...
        qrContainer.innerHTML = `
            <div class="text-center text-red-500">
                ...
                <p class="text-xs">Failed to load QR</p>
            </div>
        `;
    }
}
    // =============================================
    // SEARCH AND FILTER FUNCTIONS (CLIENT-SIDE)
    // =============================================
    ...
    function performClientSideSearch() {
        ...
        cards.forEach(card => {
            ...
            // Department filter
            if (show && departmentId && employee.department_id != departmentId) {
                show = false;
            }

            // Unit filter
            if (show && unitId && employee.unit_id != unitId) {
                show = false;
            }
        ...
        if (searchTerm || departmentId || unitId || status) {
        ...
        // Update total count
        document.getElementById('totalQrCount').textContent = visibleCount;
    }
// Generate QR codes sequentially to prevent 503 errors
document.addEventListener('DOMContentLoaded', async function() {
    ...
    console.log(`Loading QR codes for ${containers.length} employees sequentially...`);

    for (const container of containers) {
        const employeeId = container.id.replace('qrcode-', '');

        try {
            await generateEmployeeQRCode(employeeId);
            successCount++;
        } catch (error) {
            failCount++;
        }
    ...
    console.log(`QR loading complete: ${successCount} successful, ${failCount} failed`);
});
    // =============================================
    // DOWNLOAD FUNCTIONS
    // =============================================
    ...
    async function downloadMealCard(employeeId) {
        ...
        showLoading();
        updateProgressText('Generating PDF...');
  ...
  // Function to download all meal cards as a ZIP file - FIXED VERSION
async function downloadAllMealCards() {
    // Get visible employees - FIXED SELECTOR
    const visibleCards = document.querySelectorAll('.employee-card:not([style*="display: none"])');
    const visibleEmployees = Array.from(visibleCards).map(card => {
    ...
    showLoading();
    updateProgressText(`Preparing ${visibleEmployees.length} PDFs...`);
    ...
    try {
        ...
        for (let i = 0; i < visibleEmployees.length; i++) {
            const employee = visibleEmployees[i];
            ...
            try {
                ...
                // Generate PDF as blob with timeout
                const pdfBlob = await generatePDFBlob(data.qr_data, employee).catch(error => {
                    console.error(`Error generating PDF for employee ${employee.id}:`, error);
                    return null;
                });

                if (pdfBlob) {
                    // Sanitize filename
                    const fileName = `MealCard_${(data.qr_data.formal_name || employee.formal_name || `Employee_${employee.id}`).replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
            ...
            updateProgressText(`Generating PDFs... (${i + 1}/${visibleEmployees.length})`);
...
// Function to download today's uploaded meal cards - FIXED
async function downloadTodaysMealCards() {
    ...
    showLoading();
    updateProgressText(`Found ${todaysEmployees.length} employee(s) uploaded today. Preparing download...`);
    ...
    try {
        ...
        for (let i = 0; i < todaysEmployees.length; i++) {
            ...
            try {
                ...
                if (pdfBlob) {
                    // Sanitize filename
                    const fileName = `MealCard_${(data.qr_data.formal_name || employee.formal_name || `Employee_${employee.id}`).replace(/[^a-zA-Z0-9]/g, '_')}_${employee.employee_code}.pdf`;
                    zip.file(fileName, pdfBlob);
                    downloadCount++;
                } else {
                    failedCount++;
            ...
            // Update loading message
            updateProgressText(`Generating PDFs... (${i + 1}/${todaysEmployees.length})`);
        ...
        alert(`Successfully downloaded ${downloadCount} PDF meal cards from today (${today})!`);
 ...
 // Helper function to generate PDF as blob - ADD ERROR HANDLING
async function generatePDFBlob(qrData, employee) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Add Reeds Logo (with try-catch)
            try {
                if (REEDS_LOGO_BASE64) {
                    doc.addImage(REEDS_LOGO_BASE64, 'PNG', logoX, logoY, logoWidth, logoHeight);
                }
            } catch (e) {
                console.warn('Could not add Reeds logo:', e);
            }
            ...
            doc.text("OFFICIAL MEAL CARD", pageWidth / 2, titleY, { align: 'center' });
            ...
            try {
                ...
                setTimeout(() => {
                    ...
                    try {
                        ...
                        doc.text(`Employee No: ${qrData.employee_code || employee.employee_code || 'N/A'}`, boxX + PADDING, currentY);
                        currentY += 50;
                        doc.text(`Name: ${qrData.formal_name || employee.formal_name || 'N/A'}`, boxX + PADDING, currentY);
                        currentY += 50;
                        doc.text(`Designation: ${qrData.designation || employee.designation || 'N/A'}`, boxX + PADDING, currentY);
                        currentY += 50;
                        doc.text(`Department: ${qrData.department || (employee.department?.name) || 'N/A'}`, boxX + PADDING, currentY);
                    } catch (e) {
                        console.warn('Could not add employee info to PDF:', e);
                    }

                    // Add footer
                    try {
                        if (BIZTRAK_LOGO_BASE64) {
                            const img = new Image();
                            ...
                            img.onload = function() {
                                try {
                                    ...
                                    console.warn('Could not add footer with logo:', e);
                                    // Still resolve with PDF even if footer fails
                                    const pdfBlob = doc.output('blob');
                                    resolve(pdfBlob);
                                }
                            };
                ...
                }, 500); // Increased timeout to 500ms for QR generation
...
async function generateAndDownloadPDF(qrData, employee) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Store logo references locally to ensure they're in scope
            const reedsLogo = REEDS_LOGO_BASE64;
            const biztrakLogo = BIZTRAK_LOGO_BASE64;
            ...
            // Add Reeds Logo (with error handling)
            try {
                if (reedsLogo) {
                    doc.addImage(reedsLogo, 'PNG', logoX, logoY, logoWidth, logoHeight);
                }
            } catch (e) {
                console.warn('Could not add Reeds logo:', e);
            }
                    ...
                    if (biztrakLogo) {
                        ...
                        img.onload = function() {
                            try {
                                ...
                                doc.addImage(biztrakLogo, 'PNG', logoX, logoY, logoWidth, logoHeight);
                                doc.setTextColor(107, 114, 128);
                                doc.setFontSize(18);
                                doc.text('Powered By: www.biztrak.ke', PADDING, logoY + (logoHeight / 2));

                                // Save PDF
                                const fileName = `MealCard_${(qrData.formal_name || employee.formal_name || 'Employee').replace(/\s+/g, '_')}.pdf`;
                                doc.save(fileName);
                                resolve();
                            } catch (e) {
                                console.warn('Could not add footer with logo:', e);
    ...
    function printQRCodes() {
        alert('Print functionality would generate a PDF with all QR codes for printing.');
    }
// Add this function to check if all libraries are loaded
