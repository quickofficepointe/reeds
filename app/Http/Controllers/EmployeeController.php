...
public function exportScanData(Request $request)
{
    ...
    return view('reeds.admin.employees.scan-data-export', compact('startDate', 'endDate'));
}

/**
 * Generate and export scan data report
 */
public function generateScanDataReport(Request $request)
{
    ...
    // Get all employees with their scan data
    $employees = Employee::with(['department', 'unit'])
        ->where('is_active', true)
        ->get();
...
{
    $filename = 'employee_scan_data_' . $data['start_date'] . '_to_' . $data['end_date'] . '.csv';
...
private function exportScanDataPDF($data)
{
    ...
    $filename = 'employee_scan_data_' . $data['start_date'] . '_to_' . $data['end_date'] . '.pdf';
